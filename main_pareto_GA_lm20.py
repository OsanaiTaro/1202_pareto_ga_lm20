...
def create_genom(length):
    ...
    return ga.genom(genome_list, 0, 0, 0)#評価値0のgenomオブジェクト_osanai
...
def evaluation_1(li):
    ...
    #画像生成
    cv2.imwrite('1202_pareto_lm20_%d.png' % current_generation_individual_genomlist.index(li), img)
    #cv2.imwrite('../map_evaluate/debug/uc_taromap_noprop_%d.png' % current_generation_individual_group.index(ga), img)

    #Nscore算出！！！！
    sample = es.evaluate_sample(urg_pix=400, geomap='1202_pareto_lm20_%d.png' % current_generation_individual_genomlist.index(li))
...
def select(ga, elite):
    ...
    sort_result = sorted(ga, key=lambda u: u.rank)
    # 一定の上位を抽出する
...
def crossover(ga_one, ga_second):
    ...
    genom_list.append(ga.genom(progeny_one, 0, 0, 0))
    genom_list.append(ga.genom(progeny_second, 0, 0 ,0))
    return genom_list
...
def next_generation_gene_create(ga, ga_progeny):
    ...
    next_generation_geno = sorted(ga, reverse=True, key=lambda u: rank)
    # 追加する子孫集団の合計ぶんを取り除く
    for i in range(0, len(ga_progeny)):
        next_generation_geno.pop(0)
    # 子孫集団を次世代集団へ追加します
    next_generation_geno.extend(ga_progeny)#子孫集団をリストの最初に追加_osanai
...
if __name__ == '__main__':
    ...
    for count_ in range(1, MAX_GENERATION + 1):
        ...
        #print("子供個体は" + str(progeny_gene))#6個体いるはず_osanai
        ...
        #print("次世代個体は" + str(next_generation_individual_group))
        ...
        file = open('1202_all_1.txt', 'a')
        file.write(str(count_) + " " + str(fits_1) + "\n")
        file.close()

        file = open('1202_all_2.txt', 'a')
        file.write(str(count_) + " " + str(fits_2) + "\n")
        file.close()

        file = open('1202_min_1.txt', 'a')
        file.write(str(count_) + " " + str(min_1) + "\n")
        file.close()

        file = open('1202_max_1.txt', 'a')
        file.write(str(count_) + " " + str(max_1) + "\n")
        file.close()

        file = open('1202_avg_1.txt', 'a')
        file.write(str(count_) + " " + str(avg_1) + "\n")
        file.close()

        file = open('1202_minmap_1.txt', 'a')
        file.write(str(count_) + " " + str(elite_genes[0].getGenom()) + "\n")
        file.close()

        file = open('1202_minmap_1_landmark_number.txt', 'a')
        file.write(str(count_) + " " + str(sum(eval(str(elite_genes[0].getGenom()).rstrip()))) + "\n")
        file.close()
        # 現行世代と次世代を入れ替えます
